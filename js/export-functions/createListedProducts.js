export function createHTML(container, array) {
    ...
    for (let i = 0; i < array.length; i++) {
        if (array[i].featured_media) {
            ...
            <img class="featured_img_thumbnail" src="${array[i]._embedded['wp:featuredmedia']['0'].media_details.sizes.medium.source_url}" alt="${array[i]._embedded['wp:featuredmedia']['0'].alt_text}" title="${array[i]._embedded['wp:featuredmedia']['0'].alt_text}">
            </a>
            <a class="link-on-thumbnail" href="singleblogpost.html?id=${array[i].id}">
